...
class GraspData(data.Dataset):
    def __init__(self, dataroot, img_size=(224, 224), seg_th=0.002, split='train',
        ...
        file_path = os.path.join(dataroot, '%s_set.csv' % split)
    ...
    def __getitem__(self, index):
        ...
        pc_index = pc_index[np.logical_not(del_idx)]
        ...
        grid_un_choose = np.where(grid_choose is False)[0]
    ...
    def read_grasps(self, shape):
        if self.split != 'test':
            ...
            nega_idx = np.nonzero((labels is False).reshape(-1))[0].reshape(-1)
