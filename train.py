...
parser.add_argument('--dataset_root', type=str, default='/home/zibo/data_root/GPNet_release_data/',
...
parser.add_argument('--logdir', default='/home/zibo/GPNet/log_exp', type=str, metavar='SPATH',
...
def main():
    ...
    outputdir = os.path.join(opt.logdir, 'gridlen%r_gridnum%d' % (opt.grid_len, opt.grid_num),
                             'bs%d_wd%s_lr%r_lamb%r_ratio%r_posi%r_%s' % (
                                 opt.batch_size, opt.wd, opt.lr, opt.lamb, opt.ratio, opt.posi_ratio, opt.optimizer))
    if not os.path.exists(outputdir):
        os.makedirs(outputdir)
    LOG_FOUT = open(os.path.join(outputdir, 'log.txt'), 'w')
    log_other = open(os.path.join(outputdir, 'log_others.txt'), 'w')
    ...
    for epoch in range(start_epoch, opt.nepoch + 1):
        ...
        for i, data in enumerate(dataLoader):
            pc_, grids_, contact_, center_, contact_index_, scores_, grasps_idx_, angles_, posi_mask_, angles_scorer_, \
            posi_nega_idx_ = data
            ...
            pc1, grids1, contact_index1, center1, scores1 = pc_.float().cuda(1), grids_.float().cuda(1), \
                                                            contact_index_.long().cuda(1), center_.float().cuda(1), \
                                                            scores_.float().cuda(1)
            ...
            posi_idx, nega_idx = pairs_all_.cuda(0), scores_all_.cuda(0), offsets_all_.cuda(0), local_points_.cuda(0), \
                                 data_index_.cuda(0), prop_label_.cuda(0), posi_prop_idx_.cuda(0), nega_prop_idx_.cuda(
                0), \
                                 posi_idx_.cuda(0), nega_idx_.cuda(0)

            # print('proposal time: ', time.time() - st, 'posi-nega num: ', posi_idx.size(0), nega_idx.size(0))
            log_other.write('proposal time: {} posi-nega num: {}\n'.format(posi_idx.size(0), nega_idx.size(0)))
            ...
            # print('proposal score: ', prop_score.max().item(), prop_score.min().item(), prop_label.max().item(),
            #       prop_label.min().item())
            log_other.write('proposal score: max {}, min {}, proposal label: max {}, min {}\n'
                            .format(prop_score.max().item(), prop_score.min().item(), prop_label.max().item(),
                                    prop_label.min().item()))
            # print('grasp score: ', pred_score.max().item(), pred_score.min().item(), gt_score.max().item(),
            #       gt_score.min().item())
            log_other.write('grasp score: max {}, min {}, gt label: max {}, min {}\n'
                            .format(pred_score.max().item(), pred_score.min().item(), gt_score.max().item(),
                                    gt_score.min().item()))
            ...
            log_other.write('angle: min {:.6f},  max{:.6f} , offset: min {:.6f}, max {:.6f}\n'.format
                            (pred_angle.min().item(), pred_angle.max().item(), pred_offset.min().item(),
                             pred_offset.max().item()))
            log_other.write('\n\n')
            log_other.flush()

            posi_gt = torch.nonzero(gt_score.view(-1)).view(-1)
            posi_score = gt_score[0, posi_gt]
            ang_loss = angle_loss(pred_angle[0][posi_gt].unsqueeze(-1), gt_angle[posi_gt].unsqueeze(-1),
