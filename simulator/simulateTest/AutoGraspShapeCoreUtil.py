...
from simulator.simulateTest.AutoGraspSimpleShapeCore import AutoGraspSimple
from simulator.simulateTest.gripperPoseTransform import matrix2quaternion
...
class AutoGraspUtil(object):
    ...
    def parallelSimulation(self, logFile, objMeshRoot, processNum, gripperFile):
        runningObjIdList, runningObjIndexList, runningAnnotaionList, runningAnnotaionIndexList = self.__getRunningList()
        # erase log file
        open(logFile, 'w').close()
        objMeshRoot = objMeshRoot
        with Parallel(n_jobs=processNum, backend='multiprocessing') as parallel:
    ...
    def __getRunningList(self):
        ...
        return runningObjIdList, runningObjIndexList, runningAnnotaionList, runningAnnotaionIndexList

    @staticmethod
    def testAnnotation(objId, objIndex, annotation, annotationIndex, gripperFile, logfile, objMeshRoot):
    ...
    @staticmethod
    def annotationSimulation(objId, annotation, objMeshRoot, gripperFile, visual=True):
        length = annotation[0]
