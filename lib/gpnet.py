...
from lib.etw_pytorch_utils import pytorch_utils as pt_utils
from lib.pointnet2.utils.pointnet2_modules import (
...
class GraspPoseNet(nn.Module):
    def __init__(self, local_point_num=100, training=True, tanh=False, grid=False, bn=False,
                 posi_rate=0.5, use_angle=True):
        ...
        self.point_feat = PointNetFeat(bn=bn)  # point cloud feature
        dim = 128
        self.dim = dim
    ...
    def forward(self, pc, prop_local_points, prop_pair, posi_prop_idx=None, nega_prop_idx=None, posi_idx=None, \
                nega_idx=None, grasp_centers=None, grasp_angles=None, grasp_local_points=None,
                scale=0.022 * np.sqrt(3)):
        '''
        pc: (B, N, 3)
        '''
        st = time.time()
        bs = pc.size(0)
        pc_feat = self.point_feat(pc)  # (B, C, N), get point cloud feature
        ap_x = pc_feat
        C = ap_x.size(1)
        ft = time.time() - st

        if self.training:
            local_points = prop_local_points   # N(p) local points
            ...
            for i in range(num_proposal):
                ...
                gather_feat = torch.gather(ap_x, 2,
                                           points.view(bs, 1, -1).expand(-1, C, -1))  # (bs, C, num_local_point)
                if self.grid:
            ...
            _, pred_offset_batch, pred_angle_batch = self.posePred(gather_feat_batch[:, :, :posi_prop_num],
                                                                   local_pc_batch)
            ...
            for i in range(num_proposal):
                ...
                if self.grid:
                    ...
                    gather_feat = grid_feat_.expand(-1, C, local_point_num) + gather_feat
                feat_list.append(gather_feat)
                if (i + 1) % 2000 == 0 or (i + 1) == num_proposal:
                    gather_feat_batch = torch.stack(feat_list, 2).contiguous()
                    prop_score = self.ap(gather_feat_batch, local_pc_batch)
                    ...
                    time.sleep(_second)
                    _output.write('\rcomplete percent: %d/%d\t proposal positive percent: %d/%d' % (
                    i + 1, num_proposal, prop_num, num_proposal))
